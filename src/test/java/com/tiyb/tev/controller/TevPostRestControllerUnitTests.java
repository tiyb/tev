...
/**
 * <p>
 * Test cases for unit testing the REST controller for working with Posts. Not calling every single
 * method/API in the controller, just the ones that involve more logic. (i.e. we're not testing the
 * underlying Spring Boot capabilities, only the application's logic.)
 * </p>
 *
 * <p>
 * <code>updateXXX()</code> APIs <i>are</i> tested, however, to ensure that all fields are always
 * accounted for, since this is an easy place to make a mistake. The logic is always the same for
 * these tests: 1) Create empty post with hard-coded ID; 2) update the fields on that post; 3) use
 * the REST controller to update the post in the DB; 4) use the REST API to retrieve that same post;
 * 5) verify all of the fields are the same
 * </p>
 *
 * <p>
 * This class has test cases for single-blog scenarios, and gets deeper in individual parts of the
 * functionality; the {@link com.tiyb.tev.controller.TevMultiBlogPostUnitTests
 * TevPostRestControllerUnitTestsMultiBlogs} class has unit tests for multi-blog scenarios.
 * </p>
 *
 */
...
public class TevPostRestControllerUnitTests {
    ...
    /**
     * Verify that marking a post a "favourite" in the DB really does mark it as a favourite
     */
    @Test
    ...
    /**
     * Verify that marking a post not a "favourite" in the DB really does mark it as not a favourite
     */
    @Test
    ...
    /**
     * Verifies that retrieving the list of Types from the system returns the proper number of items
     */
    @Test
